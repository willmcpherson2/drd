...
mod test {
    ...
    #[test]
    fn test_bool() {
        assert_eq!(parse_bool("true"), Ok(("", Bexp::Bool(true))));
        assert_eq!(parse_bool("false"), Ok(("", Bexp::Bool(false))));
    }

    #[test]
    fn test_int() {
        assert_eq!(parse_int("123"), Ok(("", Bexp::Int(123))));
        assert_eq!(parse_int("-42hello"), Ok(("hello", Bexp::Int(-42))));
    }

    #[test]
    fn test_str() {
        assert_eq!(parse_str("''"), Ok(("", Bexp::Str("".to_string()))));
        assert_eq!(
            parse_str("'hello'"),
            Ok(("", Bexp::Str("hello".to_string())))
        );
        assert_eq!(
            parse_str("'hello'world"),
            Ok(("world", Bexp::Str("hello".to_string())))
        );
    }

    #[test]
    fn test_var() {
        assert_eq!(parse_var("x"), Ok(("", Bexp::Var("x".to_string()))));
        assert_eq!(parse_var("_x_1"), Ok(("", Bexp::Var("_x_1".to_string()))));
    }
